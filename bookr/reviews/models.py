...
class Publisher(models.Model):
    ...
    website = models.URLField(
        help_text="The publsiher's website"
    )
    email = models.EmailField(
        help_text="The publisher's email adress."
    )
    def __str__(self):
        return self.name
...
class Contributor(models.Model):
    ...
    last_names = models.CharField(
        max_length=50,
        help_text="The contributor's last name(s)."
    )
    email = models.EmailField(
        help_text="The contact email address."
    )
    def __str__(self):
